...
HI_S32 SAMPLE_COMM_IVE_CreateMemInfo(IVE_MEM_INFO_S* pstMemInfo, HI_U32 u32Size)
{
    ...
    s32Ret = HI_MPI_SYS_MmzAlloc(&pstMemInfo->u64PhyAddr, (HI_VOID**)&pstMemInfo->u64VirAddr, NULL, HI_NULL, u32Size);//在用户态分配 MMZ 内存
    if (s32Ret != HI_SUCCESS)
    {
        SAMPLE_PRT("Mmz Alloc fail,Error(%#x)\n", s32Ret);
        return HI_FAILURE;
...
HI_S32 SAMPLE_COMM_IVE_StartVo(HI_VOID)
{
    ...
    stVoPubAttr.enIntfSync = VO_OUTPUT_1080P30; //VO_OUTPUT_1080P60
