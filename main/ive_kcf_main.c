...
WK_KCF_INPUT_PRM kcf_input_prm = {0};
...
static HI_VOID *VIVO_HDMI_Showing(HI_VOID *pArgs)
{
    ...
    while (HI_FALSE == s_bVIVOStopSignal)
    {
        ...
        /*vpss 1 1920*1080 to VO  AND KCF*/  
        ...
        kcf_input_prm.stKcfFrmInfo = stBaseFrmInfo;
        ...
        WK_POINT_ARRAY_S stPoint_u = {0};
        stRect_f.u16Num = 1;
        if(kcf_input_prm.stKcfRes.stResBbox.fX == 0)
        {
            
        }
        else
        {
            stRect_f.astRect[0].stRect_f = kcf_input_prm.stKcfRes.stResBbox;
            RECT_TO_DENRMALIZATION(&stRect_f, &stRect_u, stBaseFrmInfo.stVFrame.u32Width, stBaseFrmInfo.stVFrame.u32Height);
            
            RECT_TO_POINT(&stRect_u,&stPoint_u);
            

            WK_Fill_ONE_Rect(&stBaseFrmInfo, &stPoint_u, 0x0000FF00);
        }
...
static HI_VOID *WK_YOLOV3_Detecting(HI_VOID *pArgs)
{
    ...
    while (HI_FALSE == s_bYOLOv3StopSignal)
    {
        ...
        #if 1
        /*KCF*/
        kcf_input_prm.bFlashSelectBbox = HI_TRUE;
        s32Ret = wk_kcf_proc(&kcf_input_prm);
        SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,KCF_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
	    "Error,wk_kcf_proc failed!\n");
        // if (HI_SUCCESS != s32Ret)
        // {
        //     printf("yolov3_run fault!!!\n");
        //     s_bYOLOv3StopSignal = HI_TRUE;
...
int main(int argc, char *argv[])
{
    ...
    ******************************************/
    ...
    #if 1
    s32Ret = wk_kcf_prm_init();
    if (HI_SUCCESS != s32Ret)
    {
        printf("wk_kcf_prm_init fault!\n");
    }
    else{
        printf("wk_kcf_prm_init success!!!!!!!!!\n");
    }
    #endif
    ...
    //获取框选坐标
    printf("get select Object bbox\n");
    //stSotKcf_Prm..u32GetTrackNum = 1;
    //stSotKcf_Prm.stSlectBboxInfo.s24q8X
    //s_stIveKcfInfo.astGetTrackInfo[0].u32RoiId = 1;
    sscanf(argv[1], "%f", &kcf_input_prm.stInputBboxInfo.fX);  //0.104
    sscanf(argv[2], "%f", &kcf_input_prm.stInputBboxInfo.fY);  //0.185
    sscanf(argv[3], "%f", &kcf_input_prm.stInputBboxInfo.fWidth);
    sscanf(argv[4], "%f", &kcf_input_prm.stInputBboxInfo.fHeight);
    kcf_input_prm.bFlashSelectBbox = HI_TRUE;
